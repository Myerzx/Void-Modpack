...
export default function DatapacksPage() {
  ...
  return (
    ...
    >
      ...
      {quality === 'stored' ? <>
        <section className="stat-strip"><div><strong className="stat-value">{packs.length}</strong><span className="stat-label">datapacks</span></div><div><strong className="stat-value">{resourceCount}</strong><span className="stat-label">recursos</span></div><div><strong className="stat-value">{overrideCount}</strong><span className="stat-label">overrides comprovados</span></div><div><strong className="stat-value">{packs.reduce((sum, pack) => sum + pack.unknownCount, 0)}</strong><span className="stat-label">efeitos desconhecidos</span></div></section>
        ...
        <section className="card-grid">{visible.map((pack) => <article className="card datapack-card" key={pack.datapackId}><div className="card-head"><div><span className="eyebrow">{pack.loader}</span><h2>{pack.name}</h2></div><span className="tag">{pack.resourceCount} recursos</span></div><code>{pack.rootPath}</code><div className="datapack-effects"><span><strong>{pack.overrideCount}</strong> overrides</span><span><strong>{pack.extensionCount}</strong> extensões</span><span><strong>{pack.unknownCount}</strong> desconhecidos</span></div><div className="chip-list">{pack.namespaces.map((namespace) => <span className="tag" key={namespace}>{namespace}</span>)}</div><div><span className="eyebrow">Tipos principais</span><ul className="bar-list">{pack.resourceTypes.slice(0, 6).map(([type, count]) => <li key={type}><span>{type}</span><strong>{count}</strong></li>)}</ul></div><p className="subtle">Mods relacionados: {pack.relatedModIds.length === 0 ? 'não determinados' : pack.relatedModIds.join(', ')}</p></article>)}</section>
      </> : null}
    </PanelShell>
  );
}
