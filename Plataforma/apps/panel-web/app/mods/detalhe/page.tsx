...
export default function ModDetailPage() {
  ...
  return (
    ...
    >
      ...
      {tab === 'geral' ? (
        <>
          <section className="stat-strip"><div><strong className="stat-value">{detail.configurations.length}</strong><span className="stat-label">configurações</span></div><div><strong className="stat-value">{detail.systems.length}</strong><span className="stat-label">sistemas</span></div><div><strong className="stat-value">{detail.datapacks.length}</strong><span className="stat-label">datapacks relacionados</span></div><div><strong className="stat-value">{functionalRelationships.length}</strong><span className="stat-label">relações funcionais</span></div></section>
      ...
      {tab === 'configuracoes' ? (
        <section className="config-workbench">
          <aside className="system-index card"><span className="eyebrow">Sistemas</span><strong>{detail.configurations.length} configurações</strong><nav>{detail.systems.filter((system) => (configurationsBySystem.get(system.systemId)?.length ?? 0) > 0).map((system) => <a key={system.systemId} href={`#${system.systemId}`}>{system.title}<span>{configurationsBySystem.get(system.systemId)?.length ?? 0}</span></a>)}</nav></aside>
      ...
      {tab === 'integracoes' ? <section className="card"><div className="card-head"><div><h2>Relações comprovadas</h2><p className="subtle">A direção da seta é preservada do modelo normalizado.</p></div><span className="tag">{functionalRelationships.length} relações</span></div>{functionalRelationships.length === 0 ? <p className="muted">Nenhuma relação funcional foi comprovada nesta análise.</p> : <div className="relationship-list">{functionalRelationships.map((relationship) => <article key={relationship.relationshipId}><div className="relation-flow"><code>{relationship.from.id}</code><span>{RELATION_LABELS[relationship.type] ?? relationship.type}</span><code>{relationship.to.id}</code></div><p>{relationship.reason}</p><div className="config-meta"><span>{relationship.status}</span><span>confiança {relationship.confidence}</span><span>{relationship.evidenceIds.length} evidência(s)</span></div></article>)}</div>}</section> : null}
