...
21. `npm run check` typechecks before it builds apps. A test that imports a sibling app must use a relative source path, never the package entry point, because `apps/*/dist` does not exist on a clean checkout. Verify such a change by deleting every `apps/*/dist` before running the gate.
