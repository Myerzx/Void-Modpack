...
- `GuardedDatapackLoadOrderObserver` com reader confiável injetado, fixture sanitizada, fonte fixa, timestamp do host e captura somente dentro de `offline-exclusive-v1`;
- reader do Server Agent construído exclusivamente da raiz da workspace `server` vinculada e do literal `world/level.dat`, com contenção, recusa de links e leitura máxima de 8 MiB;
...
- prova E2E sintética que percorre `POST` autenticado → job durável → claim/lease assinado → supervisor/handler reais → `level.dat` temporário limitado → observação e auditoria persistidas, incluindo replay sem segunda leitura;
...
- matriz CI de 2026-08-12 aprovada em `ubuntu-latest` e `windows-latest` pela primeira vez desde 2026-08-05: [execução 31566010444](https://github.com/Myerzx/Void-Modpack/actions/runs/31566010444), 12m21s, incluindo gate completo, validador documental e auditoria de runtime. A execução anterior, [31564990122](https://github.com/Myerzx/Void-Modpack/actions/runs/31564990122), já provou a correção da ordem: o Linux passou e o Windows chegou a rodar o gate inteiro por 12m23s em vez de morrer em 3m30s no typecheck. Ela expôs um defeito que a CI nunca tinha alcançado, porque os commits de `artifact.install` estavam entre os 22 retidos: a instalação comparava a grafia da raiz com seu `realpath`, e o runner Windows resolve seu diretório temporário por alias curto 8.3, então recusava um servidor saudável com `precondition-not-met`. A comparação passou a ser por dispositivo e inode, como `authorized-files` já fazia, e foi a única ocorrência do padrão no repositório;
...
- Graphify atualizado para 6.915 nós, 12.128 arestas e 432 comunidades, conectando a prova E2E ao contrato/produtor da Control API, fila/transporte, capability/handler, reader literal, persistência e readiness; zero endpoint ausente, zero duplicata e somente as duas autociclagens SQL já conhecidas, sem copiar dados privados do runtime;
