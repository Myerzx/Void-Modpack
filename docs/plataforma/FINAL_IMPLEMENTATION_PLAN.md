...
- **o Forge 1.20.1 não tem JAR gordo.** O `run.sh` do servidor real é `java @user_jvm_args.txt @libraries/net/minecraftforge/forge/1.20.1-47.4.4/unix_args.txt nogui`, então um plano com `-jar` não inicia coisa alguma. `createForgeArgsFileProcessPlan` cobre essa forma, e o formato de lançamento é **nomeado**, não adivinhado;
