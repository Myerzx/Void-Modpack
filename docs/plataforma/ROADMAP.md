...
A fronteira de ordem efetiva agora inclui contrato, captura offline guardada, persistência isolada e reader gzip/NBT limitado para `Data.DataPacks`. A direção de prioridade e o ID `data/<nome>` foram comprovados nos artefatos públicos Minecraft/Forge/OpenLoader 1.20.1, e o corpus versionado é inteiramente sintético. Capability, handler, auditoria operacional, leitura de filesystem real e mudança no gate de edição continuam pendentes; nenhum mundo privado foi lido.
