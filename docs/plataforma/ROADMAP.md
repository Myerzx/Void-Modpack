...
A fronteira de ordem efetiva agora inclui contrato, captura offline guardada, persistência isolada, reader gzip/NBT limitado para `Data.DataPacks`, composição operacional no Server Agent e produtor tipado na Control API. A raiz é resolvida da workspace `server` vinculada, o único arquivo permitido é `world/level.dat`, e request/job/capability/handler/readiness, replay e auditoria não carregam paths ou bytes. `datapacks.observe` é concedida somente a owner/administrator e permanece separada do grant da capability. Ensaio E2E sintético do produtor ao handler, status/painel somente leitura, grant operacional explícito, smoke do mundo privado e qualquer mudança no gate de edição continuam pendentes; nenhum mundo privado foi lido.
