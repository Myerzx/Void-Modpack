...
10. **Fase 17 — operação do servidor no painel:** lifecycle, detecção de runtime, vínculo de instância, agente local, ownership durável, [console ao vivo](PHASE_17_LIVE_CONSOLE.md), `artifact.install` e [backup local cifrado](PHASE_17_LOCAL_BACKUPS.md) concluídos. O ensaio não destrutivo `backup.verify-restore` existe: ele restaura para uma raiz privada, monta o runtime em volta da cópia e a inicia na loopback, sem tocar no mundo ativo. Ele ainda não foi comprovado em execução real, porque o teto de lease do transporte (930 s, sem renovação) é menor que uma cópia multi-GB somada a um boot do Forge já observado em 723 s. Restore do mundo ativo permanece bloqueado — ver [plano](PLANO_INICIAR_SERVIDOR.md).
