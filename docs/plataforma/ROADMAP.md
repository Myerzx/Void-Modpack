...
10. **Fase 17 — operação do servidor no painel:** lifecycle, detecção de runtime, vínculo de instância, agente local, ownership durável e [console ao vivo](PHASE_17_LIVE_CONSOLE.md) concluídos. Backup/restore operacional e `artifact.install` permanecem pendentes — ver [plano](PLANO_INICIAR_SERVIDOR.md).
...
A fronteira de ordem efetiva agora inclui contrato, captura offline guardada, persistência isolada, reader gzip/NBT limitado para `Data.DataPacks` e composição operacional no Server Agent. A raiz é resolvida da workspace `server` vinculada, o único arquivo permitido é `world/level.dat`, e capability/handler/readiness, replay por job e auditoria transacional não carregam paths ou bytes. A direção de prioridade e o ID `data/<nome>` foram comprovados nos artefatos públicos Minecraft/Forge/OpenLoader 1.20.1, e o corpus versionado é inteiramente sintético. Produtor na Control API, RBAC/painel, smoke do mundo privado e qualquer mudança no gate de edição continuam pendentes; nenhum mundo privado foi lido.
