...
10. **Fase 17 — operação do servidor no painel:** detecção de runtime (Forge, NeoForge, Fabric, Paper, Spigot, vanilla), instância com diretório de execução, agente no ambiente local, e telas de processo, console e backups. O motor existe desde as Fases 9 e 10 e nunca foi ligado — ver [plano](PLANO_INICIAR_SERVIDOR.md).
