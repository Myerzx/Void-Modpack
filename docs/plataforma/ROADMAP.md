...
11. **Fase 18 — servidor e mundo:** primeira fatia de `server.properties` concluída com schema revisado de autenticação/whitelist/RCON, preservação opaca, bootstrap e painel; demais propriedades, gamerules e gerenciamento de mundo (ativo, importar, trocar, criar, remover, backup) permanecem sob guarda offline e pendentes. Consulte [Fase 18 — segurança do server.properties](PHASE_18_SERVER_PROPERTIES_SECURITY.md).
